...
4. **Action Button binding**: ship instructions in onboarding to map Action Button to a Shortcut that opens the app at the capture route. (iOS doesn't allow direct app-level Action Button binding; a one-step Shortcut is the standard path.)
5. **Background sync**: `URLSession` with background configuration. Each pending memory is one upload task; success transitions `pending → synced`; failure with retryable error → backoff; non-retryable (4xx other than 401) → `failed` with error visible in a debug screen.
6. **Sweep on launch + on `NWPathMonitor` path satisfied**: re-enqueue all `pending`/`failed` items.
7. **Keychain** stores the bearer token + server URL; settings screen lets you paste them on first run.
