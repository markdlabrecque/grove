...
Invoked via ``python -m grove.enrichment.run`` by cron (hourly).
...
async def run(
    ...
    Args:
        ...
        classify_and_write: Async callable invoked once per memory with the
            ...
            to the real orchestrator implementation from grove.enrichment.orchestrator.
    """
    if classify_and_write is None:
        from grove.enrichment.orchestrator import (
