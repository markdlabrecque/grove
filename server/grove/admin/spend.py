...
Callers
-------
grove.api.queries.post_query — checks before calling synthesize() and
    classify_intent(). On SpendCapExceededError the endpoint returns answer=None
    (ranked-snippets-only degradation).

grove.enrichment.orchestrator.classify_and_write — checks before calling the
...
async def check_spend_cap(
    ...
    if override_settings is None:
        from grove.core.config import settings as _settings
