...
// MARK: - GroveAPIListTasksTests
//
// Tests for `GroveAPI.listTasksByMemoryIDs(_:)` (#404):
//   1. Happy path: stub returns 200 with a [TaskDTO] array → decoded correctly.
//   2. Empty input: no network call made, returns [].
//   3. URL shape: memory_ids joined as comma-separated query string.
...
@Suite("GroveAPI — listTasksByMemoryIDs")
struct GroveAPIListTasksTests {

  // MARK: - Fixtures

  private static let baseURL = URL(string: "https://grove.example.ts.net")!
  private static let token = "list-tasks-token"
  ...
  /// Build a minimal JSON array of two TaskDTO objects for use in stub responses.
  ...
  @Test("listTasksByMemoryIDs: stub returns 200 with two TaskDTOs → decoded correctly")
  func happyPath() async throws {
    ...
    let result = try await api.listTasksByMemoryIDs([Self.memoryID1, Self.memoryID2])
  ...
  @Test("listTasksByMemoryIDs: empty input returns [] without making a network call")
  func emptyInputNoNetworkCall() async throws {
    ...
    let result = try await api.listTasksByMemoryIDs([])
  ...
  @Test("listTasksByMemoryIDs: memory_ids query string is comma-joined UUIDs")
  func urlShape() async throws {
    ...
    _ = try await api.listTasksByMemoryIDs([Self.memoryID1, Self.memoryID2])
    ...
    let memoryIDsParam = comps?.queryItems?.first(where: { $0.name == "memory_ids" })?.value
    #expect(memoryIDsParam != nil, "memory_ids query parameter must be present")

    // Both UUIDs should appear in the comma-separated value (order-agnostic check).
    let paramValue = try #require(memoryIDsParam)
    let parts = paramValue.split(separator: ",").map(String.init)
    #expect(parts.count == 2)
    #expect(parts.contains(Self.memoryID1.uuidString.lowercased()))
    #expect(parts.contains(Self.memoryID2.uuidString.lowercased()))
  }

  // MARK: - 422 error

  @Test("listTasksByMemoryIDs: 422 response throws APIError.httpError(422, _)")
  func http422ThrowsError() async throws {
    let errorBody = Data(#"{"detail": "invalid UUID in memory_ids: 'not-a-uuid'"}"#.utf8)
    ...
    do {
      _ = try await api.listTasksByMemoryIDs([Self.memoryID1])
  ...
  @Test("listTasksByMemoryIDs: request includes Authorization header")
