...
struct JSONCodingTests {
  ...
  func queryResponseBodyFirstResultIsWholeMatch() throws {
    ...
    let first = try #require(response.sources.first)
    ...
    #expect(first.excerpt.hasPrefix("The project uses SwiftData"))
  }
  ...
  func queryResponseBodySecondResultIsChunkMatch() throws {
    ...
    let second = try #require(response.sources.dropFirst().first)
  ...
  func queryResponseBodyCapturedAtIsTimezoneAware() throws {
    ...
    let capturedAt = try #require(response.sources.first?.capturedAt)
  ...
  private func loadFixture(named name: String) throws -> Data {
    ...
    guard let url = Bundle.module.url(forResource: name, withExtension: "json") else {
      Issue.record("Fixture '\(name).json' not found in Bundle.module. Verify it is under Tests/GroveCoreTests/Fixtures/ and listed in Package.swift resources.")
