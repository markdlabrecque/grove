...
/// `OracleAPI.completeTask(identifier:response:error:)` once the task reaches
...
/// background URLSession and `OracleAPI`.
///
...
/// `OracleAPI` actor via `Task { await api?.handle… }`.
///
/// The `api` reference is `weak` so it doesn't extend the actor's lifetime
/// beyond its natural scope. In practice `OracleAPI.shared` is a singleton
...
final class UploadSessionDelegate: NSObject, URLSessionDataDelegate, URLSessionDelegate {
  ...
  // Set immediately after OracleAPI.shared initialises itself. A weak ref
  ...
  // but the property is mutable. Writes happen only in the OracleAPI init
  ...
  nonisolated(unsafe) weak var api: OracleAPI?
  ...
  ///
  /// Dispatches to `OracleAPI` so it can call the stored background-session
